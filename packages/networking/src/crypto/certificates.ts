...
import { deriveSecretSeeds, generateAlternativeName, signEd25519, verifyEd25519 } from '@pbnj/core'
...
export function validateCertificate(cert: JAMNPCertificate): boolean {
  try {
    ...
    const expectedName = generateAlternativeName(cert.publicKey, decodeFixedLength)
 ...
 */
export function extractAlternativeNameFromCertificate(cert: JAMNPCertificate): string {
 ...
 */
export function createCertificateFromKeyPair(
  keyPair: { publicKey: Bytes; privateKey: Bytes }
): JAMNPCertificate {
  const alternativeName = generateAlternativeName(keyPair.publicKey, decodeFixedLength)
  return generateCertificate(keyPair.publicKey, keyPair.privateKey, alternativeName)
}
 ...
 */
function generateEd25519KeyPairFromSeed(seed: Uint8Array): {
