...
//     new Map()
//   private finalizedBlock: { hash: Uint8Array; slot: bigint } | null = null
...
//   private activeStreams: Map<bigint, string> = new Map()
...
//    */
//   setLocalValidator(validatorIndex: bigint): void {
...
//    */
//   async updateFinalizedBlock(hash: Uint8Array, slot: bigint): Promise<void> {
//     this.finalizedBlock = { hash, slot }

//     // Persist to database
//     if (this.blockStore) {
//       try {
//         await this.blockStore.storeFinalizedBlock(hash, slot)
//       } catch (error) {
...
//    */
//   private extractSlotFromHeader(header: Uint8Array): bigint {
...
//     return BigInt(`0x${slotBytes.toString()}`)
//   }
...
//     // Write final block slot (4 bytes, little-endian)
//     view.setUint32(offset, Number(announcement.finalBlockSlot), true)
...
//     for (const [_validatorIndex, _streamId] of this.activeStreams) {
//       // TODO: Send stream close message
//       if (this.streamManager) {
//         const [error] = await this.streamManager.closeStream(_streamId)
//         if (error) {
//           errors.push(error)
//         }
//       }
...
//       logger.error('Error stopping block announcement protocol:', error)
//     }
...
//     logger.info('Announcing block via JIP-5 protocol', {
//       slot: blockData.slot,
...
//     const [error] = await this.streamManager.sendMessage(streamId, data)
//     if (error) {
//       logger.error(
