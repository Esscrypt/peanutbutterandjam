...
import { TicketHolderService } from '../../../infra/node/services/ticket-holder-service'
...
async function testNetworkingConnection(): Promise<void> {
  ...
  const ticketHolderService = new TicketHolderService({
  ...
  // Create a shared validator set manager with validators from genesis
  ...
  try {
    ...
    await nodeB.networkingService.sendMessage(
      BigInt(0), // Send to Alice (validator index 0)
      131 as StreamKind, // CE131 protocol kind byte
      serializedMessage,
    )
    ...
    logger.info('- Genesis manager service loaded validators from config files')
    logger.info('- Validator set manager initialized with genesis validators')
