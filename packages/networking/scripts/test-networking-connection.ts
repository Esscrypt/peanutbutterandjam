...
//   // Get the local key pair from the service
...
//   const ticketStore = new TicketStore(drizzleDb)

//   // Use shared services
//   const validatorSetManager = sharedValidatorSetManager

//   // No need to initialize or start services for each node
//   // as we're using the shared services

//   // Create protocol registry
//   const protocolRegistry = new Map()

//   // Create CE131 protocol with all required parameters
...
//   const keyPairService = new KeyPairService({
//     customSeed: process.env['VALIDATOR_SEED'] as `0x${string}`,
...
//   // Create genesis manager service
//   const genesisManagerService = new NodeGenesisManager(
//     './config/generated-chain-spec.json',
//     {
...
//     validators: initialValidators.map((v, i) => ({
//       index: i,
//       address: v.address,
...
//   // Create a shared validator set manager with manual validators since genesis is empty
//   const manualValidators = [
//     {
//       index: 0,
//       keys: {
//         bandersnatch: hexToBytes(
//           aliceCredentials.bandersnatchPublic as `0x${string}`,
//         ),
//         ed25519: hexToBytes(aliceCredentials.ed25519Public as `0x${string}`),
//       },
//     },
//     {
//       index: 1,
//       keys: {
//         bandersnatch: hexToBytes(
//           bobCredentials.bandersnatchPublic as `0x${string}`,
//         ),
//         ed25519: hexToBytes(bobCredentials.ed25519Public as `0x${string}`),
//       },
//     },
//   ]
...
//     validators: manualValidators.map((v, i) => ({
//       index: v.index,
//       ed25519: bytesToHex(v.keys.ed25519),
//       bandersnatch: bytesToHex(v.keys.bandersnatch),
//     })),
//   })
...
//   // Set up the active validator set with our test validators
//   sharedValidatorSetManager.setActiveSet(manualValidators.map((v) => v.keys))

//   // Initialize shared services
//   await eventBusService.init()
//   await ticketHolderService.init()
//   await entropyService.init()
//   await sealKeyService.init(sharedValidatorSetManager)
...
//     // Find connections from both nodes
//     const connectionsA = Array.from(
//       nodeA.networkingService.connections.entries(),
//     )
//     const connectionsB = Array.from(
//       nodeB.networkingService.connections.entries(),
//     )
...
//     // Use Node B's connection to Node A for testing
//     const [connectionId, _connection] =
...
//     // Debug: Check what validators are available in the validator set
//     logger.info('🔍 Debugging validator set:', {
//       validatorSetSize:
//         sharedValidatorSetManager.getAllConnectedValidators().size,
//       validatorIndices: Array.from(
//         sharedValidatorSetManager.getAllConnectedValidators().keys(),
//       ),
//       validatorKeys: Array.from(
//         sharedValidatorSetManager.getAllConnectedValidators().entries(),
//       ).map(([idx, keys]) => ({
//         index: idx,
...
//     logger.info(
//       `Node A connections: ${nodeA.networkingService.connections.size}`,
//     )
//     logger.info(
//       `Node B connections: ${nodeB.networkingService.connections.size}`,
//     )
...
//       connId,
//       conn,
//     ] of nodeA.networkingService.connections.entries()) {
//       logger.info(`Node A connection ${connId.slice(0, 20)}... streams:`, {
//         streamMap: (conn as any).streamMap
//           ? Object.keys((conn as any).streamMap).length
//           : 'No streamMap',
//       })
//     }
...
//       connId,
//       conn,
//     ] of nodeB.networkingService.connections.entries()) {
//       logger.info(`Node B connection ${connId.slice(0, 20)}... streams:`, {
//         streamMap: (conn as any).streamMap
//           ? Object.keys((conn as any).streamMap).length
//           : 'No streamMap',
//       })
