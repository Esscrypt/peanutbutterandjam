...
import { parseArgs, getOption } from '../utils/arg-parser'
import { isValidHex, isValidPath, isValidTimestamp } from '../utils/validation.js'
...
export async function createRunCommand(args: string[]): Promise<void> {
  ...
  const options = {
    ...
    'data-path': getOption(parsedArgs, 'data-path', '/Users/tanyageorgieva/.jamduna'),
  ...
  try {
    ...
    logger.info(`  Data directory: ${options['data-path'] || options.datadir || '~/.jamduna'}`)
