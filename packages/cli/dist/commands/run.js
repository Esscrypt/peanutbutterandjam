...
function findPeanutButterAndJamBinary() {
...
export function createRunCommand() {
    const command = new Command('run')
        ...
        .option('--chain <string>', 'Chain to run. "polkadot", "dev", or the path of a chain spec file', 'chainspec.json')
        .option('-d, --data-path <string>', 'Specifies the directory for the blockchain, keystore, and other data', '/Users/tanyageorgieva/.jamduna')
        .option('--debug <string>', 'Specifies debug flags for enhanced logging (block,guarantees,rotation,assurances,audit,da,node,quic,beefy,audit,grandpa,web,state)', 'r,g')
        .option('--dev-validator <int>', 'Validator Index (only for development)')
        .option('--external-ip <string>', 'External IP of this node, as used by other nodes to connect. If not specified, this will be guessed.')
        .option('--listen-ip <string>', 'IP address to listen on. :: (the default) means all addresses.', '::')
        .option('--peer-id <int>', 'Peer ID of this node. If not specified, a new peer ID will be generated. The corresponding secret key will not be persisted.')
        .option('--port <int>', 'Specifies the network listening port.', '40000')
        .option('--pvm-backend <string>', 'The PVM backend to use. Possible values: interpreter, compiler', 'interpreter')
        .option('--rpc-listen-ip <string>', 'IP address for RPC server to listen on. :: (the default) means all addresses.', '::')
        .option('--rpc-port <int>', 'Specifies the RPC listening port.', '19800')
        .option('--start-time <string>', 'Start time in format: YYYY-MM-DD HH:MM:SS')
        ...
        try {
            ...
            if (!pbnjPath) {
                ...
                possiblePaths.forEach((path) => {
                    logger.error(`  ${path}: ${existsSync(path) ? 'EXISTS' : 'NOT FOUND'}`);
                });
                throw new Error('PeanutButterAndJam binary not found. Please ensure it is built and accessible.');
            }
            ...
            pbnjProcess.on('error', (error) => {
                logger.error('Failed to start PeanutButterAndJam process:', error);
                process.exit(1);
            });
            pbnjProcess.on('exit', (code, signal) => {
                if (code !== null) {
                    logger.info(`PeanutButterAndJam process exited with code ${code}`);
                }
                else if (signal !== null) {
                    logger.info(`PeanutButterAndJam process was killed with signal ${signal}`);
                }
                process.exit(code || 0);
            });
            ...
            logger.error('Failed to start node:', error instanceof Error ? error.message : String(error));
