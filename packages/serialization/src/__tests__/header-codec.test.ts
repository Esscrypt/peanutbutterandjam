...
describe('JAM Header Codec Tests', () => {
  ...
  it('should decode and encode header_0 correctly', () => {
    ...
    const decodedHeader = decodeJamHeader(binaryData)
    
    // Verify the decoded header matches the JSON structure
    expect(decodedHeader.parent).toBe(jsonData.parent)
    expect(decodedHeader.parent_state_root).toBe(jsonData.parent_state_root)
    expect(decodedHeader.extrinsic_hash).toBe(jsonData.extrinsic_hash)
    expect(decodedHeader.slot).toBe(jsonData.slot)
    
    // Check epoch mark
    if (jsonData.epoch_mark) {
      expect(decodedHeader.epoch_mark).not.toBeNull()
      expect(decodedHeader.epoch_mark!.entropy).toBe(jsonData.epoch_mark.entropy)
      expect(decodedHeader.epoch_mark!.tickets_entropy).toBe(jsonData.epoch_mark.tickets_entropy)
      expect(decodedHeader.epoch_mark!.validators).toHaveLength(jsonData.epoch_mark.validators.length)
      
      // Check first few validators to avoid testing all 1024
      for (let i = 0; i < Math.min(5, jsonData.epoch_mark.validators.length); i++) {
        expect(decodedHeader.epoch_mark!.validators[i].bandersnatch).toBe(jsonData.epoch_mark.validators[i].bandersnatch)
        expect(decodedHeader.epoch_mark!.validators[i].ed25519).toBe(jsonData.epoch_mark.validators[i].ed25519)
      }
    } else {
      expect(decodedHeader.epoch_mark).toBeNull()
    }
    
    // Check winners mark (should be null for header_0)
    expect(decodedHeader.winners_mark).toBeNull()
    
    // Check offenders mark (should be empty array for header_0) 
    expect(decodedHeader.offenders_mark).toEqual([])
    
    // Encode the decoded header back to binary
    const encodedData = encodeJamHeader(decodedHeader)
  ...
  it('should handle round-trip encoding/decoding for simple header', () => {
    ...
    // Encode the header
    const encoded = encodeJamHeader(testHeader)
    
    // Decode the encoded data
    const decodedHeader = decodeJamHeader(encoded)
    
    // Verify round-trip consistency
    expect(decodedHeader.parent).toBe(testHeader.parent)
    expect(decodedHeader.parent_state_root).toBe(testHeader.parent_state_root)
    expect(decodedHeader.extrinsic_hash).toBe(testHeader.extrinsic_hash)
    expect(decodedHeader.slot).toBe(testHeader.slot)
    expect(decodedHeader.epoch_mark).toBe(testHeader.epoch_mark)
    expect(decodedHeader.winners_mark).toBe(testHeader.winners_mark)
    expect(decodedHeader.offenders_mark).toEqual(testHeader.offenders_mark)
    expect(decodedHeader.author_index).toBe(testHeader.author_index)
    expect(decodedHeader.vrf_sig).toBe(testHeader.vrf_sig)
    expect(decodedHeader.seal_sig).toBe(testHeader.seal_sig)
  })
  ...
  it('should handle header with epoch mark', () => {
    ...
    // Check epoch mark
    expect(decodedHeader.epoch_mark).not.toBeNull()
    expect(decodedHeader.epoch_mark!.entropy).toBe(testHeader.epoch_mark!.entropy)
    expect(decodedHeader.epoch_mark!.tickets_entropy).toBe(testHeader.epoch_mark!.tickets_entropy)
    expect(decodedHeader.epoch_mark!.validators).toHaveLength(testHeader.epoch_mark!.validators.length)
    
    for (let i = 0; i < testHeader.epoch_mark!.validators.length; i++) {
      expect(decodedHeader.epoch_mark!.validators[i].bandersnatch).toBe(testHeader.epoch_mark!.validators[i].bandersnatch)
      expect(decodedHeader.epoch_mark!.validators[i].ed25519).toBe(testHeader.epoch_mark!.validators[i].ed25519)
    }
