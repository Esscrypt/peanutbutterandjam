...
// Try to load the native module with various filename patterns
try {
  // Try platform-specific filename first (napi-rs generates more specific names)
  const platform = process.platform
  const arch = process.arch

  // Try common napi-rs filename patterns
  ...
  for (const filename of possibleFilenames) {
    try {
      nativeModule = require(`../rust-reed-solomon/native/${filename}`)
  ...
  // If no platform-specific file found, try generic filename
  if (!nativeModule) {
    try {
      nativeModule = require('../rust-reed-solomon/native/jam-reed-solomon.node')
