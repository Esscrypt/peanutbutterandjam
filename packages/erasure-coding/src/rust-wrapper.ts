...
export class RustReedSolomonCoder implements ErasureCoder {
  // biome-ignore lint/suspicious/noExplicitAny: Rust instance
  private nativeInstance: any
  // Import the native Rust module (will be generated by napi)
  // biome-ignore lint/suspicious/noExplicitAny: Rust module
  ...
  constructor(k = 2, n = 6) {
    ...
    try {
      this.nativeInstance = new this.nativeModule.RustReedSolomon(k, n)
  ...
  encode(
    ...
    const instance =
      actualK !== this.k || actualN !== this.n
        ? new this.nativeModule.RustReedSolomon(actualK, actualN)
        : this.nativeInstance
  ...
  decode(encodedData: EncodedData, k?: number): Uint8Array {
    ...
    const instance =
      actualK !== this.k || encodedData.n !== this.n
        ? new this.nativeModule.RustReedSolomon(actualK, encodedData.n)
        : this.nativeInstance
    ...
    try {
      ...
      const shardBuffers = shards.map((shard: Uint8Array) => Buffer.from(shard))
  ...
  recover(encodedData: EncodedData, corruptedIndices: number[]): Uint8Array {
    ...
    const instance =
      k !== this.k || n !== this.n
        ? new this.nativeModule.RustReedSolomon(k, n)
        : this.nativeInstance
...
export function testRustAgainstJAM(
  ...
  try {
    ...
    const systematicMatches = encoded.shards
      .slice(0, 2)
      .reduce((count: number, shard: Uint8Array, i: number) => {
    ...
    const parityMatches = encoded.shards
      .slice(2)
      .reduce((count: number, shard: Uint8Array, i: number) => {
 ...
 */
// biome-ignore lint/suspicious/noExplicitAny: test vectors
