;(() => {
  ...
  function highlightSrcLines() {
    ...
    let from = Number.parseInt(match[1], 10)
    let to = from
  ...
  const handleSrcHighlight = (() => {
    ...
    return (ev) => {
      let cur_line_id = Number.parseInt(ev.target.id, 10)
      if (Number.isNaN(cur_line_id) || ev.ctrlKey || ev.altKey || ev.metaKey) {
        return
      }
      ev.preventDefault()
