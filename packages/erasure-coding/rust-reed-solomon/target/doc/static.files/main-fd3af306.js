...
;(() => {
  ...
  window.searchState = {
    ...
    focus: () => {
      window.searchState.input?.focus()
    },
    defocus: () => {
      window.searchState.input?.blur()
    },
    showResults: (search) => {
    ...
    setLoadingSearch: () => {
      ...
      search.innerHTML = `<h3 class="search-loading">${window.searchState.loadingText}</h3>`
      window.searchState.showResults(search)
    },
    descShards: new Map(),
    loadDesc: async ({ descShard, descIndex }) => {
    ...
    if (savedHash.startsWith('impl-')) {
      ...
      if (splitAt !== -1) {
        ...
        onEachLazy(implElems, (implElem) => {
          ...
          return onEachLazy(
            implElem.parentElement.parentElement.querySelectorAll(
              `[id^="${assocId}"]`,
            ),
            (item) => {
  ...
  window.register_implementors = (imp) => {
    ...
    const synthetic_implementors = document.getElementById(
    ...
    if (synthetic_implementors) {
      onEachLazy(
        synthetic_implementors.getElementsByClassName('impl'),
        (el) => {
    ...
    const traitName = document.querySelector(
      '.main-heading h1 > .trait',
    ).textContent
    ...
    for (const lib of libs) {
      ...
      struct_loop: for (const struct of structs) {
        const list = struct[SYNTHETIC_IDX]
  ...
  window.register_type_impls = (imp) => {
    ...
    let trait_implementations = document.getElementById(
      'trait-implementations-list',
    )
    let trait_implementations_header = document.getElementById(
    ...
    let associatedTypes = document.querySelector(
      '.sidebar .block.associatedtype',
    )
    let associatedConstants = document.querySelector(
      '.sidebar .block.associatedconstant',
    )
    let sidebarTraitList = document.querySelector(
      '.sidebar .block.trait-implementation',
    )
    for (const impList of imp[window.currentCrate]) {
      ...
      if (outputList === null) {
        const outputListName = isTrait
          ? 'Trait Implementations'
          : 'Implementations'
        const outputListId = isTrait
          ? 'trait-implementations-list'
          : 'implementations-list'
        const outputListHeaderId = isTrait
          ? 'trait-implementations'
          : 'implementations'
        ...
        onEachLazy(templateAssocItems, (item) => {
          ...
          if (!block) {
            ...
            if (insertionReference) {
              const insertionReferenceH =
                insertionReference.previousElementSibling
  ...
  function expandAllDocs() {
    ...
    onEachLazy(document.getElementsByClassName('toggle'), (e) => {
      if (
        ...
        !hasClass(e, 'more-examples-toggle')
      ) {
        e.open = true
      }
    })
  ...
  ;(() => {
    ...
    const hideImplementations =
      getSettingValue('auto-hide-trait-implementations') === 'true'
    const hideLargeItemContents =
  ...
  window.rustdoc_remove_line_numbers_from_examples = () => {
    onEachLazy(
      document.querySelectorAll(
        '.rustdoc:not(.src) :not(.scraped-example) > .example-wrap',
      ),
  ...
  onEachLazy(
    document.querySelectorAll('.toggle > summary:not(.hideme)'),
    (el) => {
  ...
  function showTooltip(e) {
    ...
    if (!window.NOTABLE_TRAITS && notable_ty) {
      ...
      if (data) {
        ...
        throw new Error(
          'showTooltip() called with notable without any notable traits!',
        )
      }
    }
    if (
    ...
    if (notable_ty) {
      wrapper.innerHTML = `<div class="content">${window.NOTABLE_TRAITS[notable_ty]}</div>`
    } else {
  ...
  function setTooltipHoverTimeout(element, show) {
    ...
    element.TOOLTIP_HOVER_TIMEOUT = setTimeout(
      ...
      show
        ? window.RUSTDOC_TOOLTIP_HOVER_MS
        : window.RUSTDOC_TOOLTIP_HOVER_EXIT_MS,
    )
  }
  ...
  function tooltipBlurHandler(event) {
    if (
      ...
      !window.CURRENT_TOOLTIP_ELEMENT.TOOLTIP_BASE.contains(
        document.activeElement,
      ) &&
  ...
  onEachLazy(document.getElementsByClassName('tooltip'), (e) => {
    e.onclick = () => {
      e.TOOLTIP_FORCE_VISIBLE = !e.TOOLTIP_FORCE_VISIBLE
  ...
  function helpBlurHandler(event) {
    if (
      !getHelpButton().contains(document.activeElement) &&
...
<a href="${drloChannel}/rustdoc/">the rustdoc book</a>.`
  ...
  window.hidePopoverMenus = () => {
    onEachLazy(
      document.querySelectorAll('rustdoc-toolbar .popover'),
      (elem) => {
...
;(() => {
  ...
  const resize = (e) => {
    ...
    if (pos < SIDEBAR_VANISH_THRESHOLD) {
      ...
      const constrainedPos = Math.min(
        pos,
        window.innerWidth - BODY_MIN,
        SIDEBAR_MAX,
      )
  ...
  window.addEventListener('resize', () => {
    ...
    if (
      ...
      desiredSidebarSize >= window.innerWidth - BODY_MIN
    ) {
      changeSidebarSize(window.innerWidth - BODY_MIN)
    } else if (
      desiredSidebarSize !== null &&
      desiredSidebarSize > SIDEBAR_MIN
    ) {
