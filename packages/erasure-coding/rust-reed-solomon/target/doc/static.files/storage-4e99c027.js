...
const settingsDataset = (() => {
  ...
  return settingsElement?.dataset ? settingsElement.dataset : null
})()
function _nonnull(x, msg) {
  if (x === null) {
...
function _onEachLazy(lazyArray, func) {
...
function switchTheme(newThemeName, saveTheme) {
  ...
  if (builtinThemes.indexOf(newThemeName) !== -1) {
    if (window.currentTheme?.parentNode) {
    ...
    if (!window.currentTheme) {
      if (document.readyState === 'loading') {
        document.write(
          `<link rel="stylesheet" id="themeStyle" href="${newHref}">`,
        )
        window.currentTheme = (() => {
          const currentTheme = document.getElementById('themeStyle')
          return currentTheme instanceof HTMLLinkElement ? currentTheme : null
