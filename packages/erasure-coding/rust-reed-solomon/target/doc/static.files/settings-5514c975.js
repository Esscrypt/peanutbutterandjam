;(() => {
  ...
  function updateLightAndDark() {
    ...
    if (
      useSystem === 'true' ||
      (useSystem === null && getSettingValue('theme') === null)
    ) {
      showLightAndDark()
    } else {
      hideLightAndDark()
  ...
  function setEvents(settingsElement) {
    updateLightAndDark()
    onEachLazy(
      settingsElement.querySelectorAll('input[type="checkbox"]'),
      (toggle) => {
...
<div class="setting-line">\
  ...
  function buildSettingsPage() {
    ...
    const settings = [
      ...
      { name: 'Hide table of contents', js_name: 'hide-toc', default: false },
      {
  ...
  function displaySettings() {
    ...
    onEachLazy(
      settingsMenu.querySelectorAll("input[type='checkbox']"),
      (el) => {
