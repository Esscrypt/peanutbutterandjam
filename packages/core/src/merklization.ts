 ...
 */
export function merklize(keyValuePairs: KeyValuePair[], bitIndex: number = 0): Uint8Array {
  ...
  for (const [hexKey, hexValue] of Object.entries(input)) {
    ...
    if (hexValue === '') {
      ...
      if (!valueValidation.isValid) {
        throw new Error(`Invalid value format: ${valueValidation.errors[0]?.message}`)
      }
    }
 ...
 */
export function verifyTestVectors(testVectors: Array<{ input: TrieInput; output: string }>): Array<{ passed: boolean; expected: string; actual: string }> {
 ...
 */
export function validateTrieInput(input: unknown): { isValid: boolean; errors: string[] } {
  const errors: string[] = []
  ...
  for (const [key, value] of Object.entries(inputObj)) {
    ...
    if (!keyValidation.isValid) {
      errors.push(`Invalid key format for key "${key}": ${keyValidation.errors[0]?.message}`)
    }
    
    // Validate value format
    if (typeof value !== 'string') {
      errors.push(`Value must be a string for key "${key}", got ${typeof value}`)
      continue
    }
    
    const valueValidation = validateHexString(value, 'value')
    if (!valueValidation.isValid) {
      errors.push(`Invalid value format for key "${key}": ${valueValidation.errors[0]?.message}`)
    }
  }
