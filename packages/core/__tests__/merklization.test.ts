...
export function verifyTestVectors(
  ...
  return testVectors.map(({ input, output }) => {
    const [actualError, actual] = merklizeHex(input) // Use test vector mode (32-byte keys)
    if (actualError) {
      throw actualError
    }
...
describe('Merklization', () => {
  describe('Test Vector Verification', () => {
    ...
    it('should handle empty input correctly', () => {
      const [error, result] = merklizeHex({})
  ...
  //     }
  //     const [error, result] = merklizeHex(input)
  //     if (error) throw error
  ...
  //     }
  //     const [embeddedError, embeddedResult] = merklizeHex(embeddedInput)
  ...
  //     }
  //     const [regularError, regularResult] = merklizeHex(regularInput)
