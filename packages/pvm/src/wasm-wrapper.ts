...
export class PVMWasmWrapper implements WasmPvmShellInterface {
  ...
  private exitArg: number = 0 // eslint-disable-line @typescript-eslint/no-magic-numbers
  ...
  resetGeneric(program: Uint8Array, registers: Uint8Array, gas: bigint): void {
    try {
      ...
      const [error, codeBlob] = this.pvm.initializeProgram(program, argumentData)
  ...
  private restoreMemoryFromChunks(
    ...
    for (let mapOffset = 0; mapOffset < pageMap.length; mapOffset += entrySize) {
