...
export function extractBlakeHashFromStateKey(stateKeyBytes: Uint8Array): Hex {
  if (stateKeyBytes.length !== 31) {
    throw new Error(
      `Invalid state key length: expected 31, got ${stateKeyBytes.length}`,
    )
  }
...
function parseRequestTimeslots(valueBytes: Uint8Array): bigint[] | null {
  // 4 bytes for timeslots, 1 for length, 5 bytes total
...
export function matchRequestKey(
  ...
  | { preimageHash: Hex; timeslots: bigint[] }
  | { preimageHash?: Hex; timeslots: bigint[]; isPending: true }
  | null {
  ...
  if (!timeslots) {
    logger.error('MatchRequestKey: Failed to parse timeslots', {
      stateKeyHex,
      valueBytes,
    })
    return null
  ...
  if (!validateRequestTimeslots(timeslots, currentTimeslot)) {
    logger.error('MatchRequestKey: Failed to validate timeslots', {
      stateKeyHex,
      valueBytes,
      timeslots,
      currentTimeslot,
  ...
  for (const [, preimageData] of preimages) {
    if (
      isRequestKeyForPreimage(
        stateKeyHex,
        preimageData.preimageHash,
        preimageData.blob.length,
      )
    ) {
      return {
...
> {
  ...
  for (const [stateKeyHex, valueHex] of Object.entries(rawCshKeyvals)) {
    ...
    if (stateKeyBytes.length !== 31) {
      logger.warn('DetermineKeyTypes: Invalid state key length', {
        stateKeyHex,
        stateKeyBytesLength: stateKeyBytes.length,
      })
      continue
    }
    ...
    const requestMatch = matchRequestKey(
      stateKeyHex as Hex,
      valueBytes,
      preimages,
      currentTimeslot,
