 ...
 * deblob: blob → (blob, bitstring, sequence{pvmreg}) ∪ error
 *
 * p ↦ {
 *   (c, k, j)  when ∃! c, k, j : p = encode(len(j)) ⊕ encode[1](z) ⊕ encode(len(c)) ⊕ encode[z](j) ⊕ encode(c) ⊕ encode(k), len(k) = len(c)
 *   error      otherwise
 * }
 *
 * Format: p = encode(len(j)) ⊕ encode[1](z) ⊕ encode(len(c)) ⊕ encode[z](j) ⊕ encode(c) ⊕ encode(k)
 ...
 * Where:
 * - encode(len(j)): Jump table length (natural number encoding)
 * - encode[1](z): Element size in jump table (1 byte, little-endian)
 * - encode(len(c)): Code section length (natural number encoding)
 * - encode[z](j): Jump table data (each element is z bytes, little-endian)
 * - encode(c): Code data (instruction bytes)
 * - encode(k): Opcode bitmask (1 bit per byte of code, marking opcodes)
 ...
 *
 * @param programBlob - The program blob bytes
 * @returns Decoded blob components (code, bitmask, jump table) or error
 */
export function decodeBlob(
  ...
  for (let i = 0; i < jumpTableLength; i++) {
    ...
    // Decode as little-endian (Gray Paper serialization.tex line 100-108)
    // encode[l](x) = ⟨x mod 256⟩ ∥ encode[l-1](⌊x/256⌋)
    // Values are encoded in regular little-endian fashion
    let value = 0n
    for (let j = 0; j < elementSize; j++) {
      value |= BigInt(elementBytes[j]) << BigInt(j * 8)
    }
  ...
  // Validate jump table entries: all entries must be < code.length
  // Gray Paper: jump table entries are program counter values (indices into code)
  // They must be valid addresses within the code section
  // for (let i = 0; i < jumpTable.length; i++) {
  //   const entry = jumpTable[i]
  //   if (entry < 0n || entry >= BigInt(codeLength)) {
  //     return safeError(
  //       new Error(
  //         `Invalid jump table entry at index ${i}: ${entry.toString()} is out of bounds (code length: ${codeLength})`,
  //       ),
  //     )
  //   }
  // }
...
> {
  ...
  const heapZeroPaddingSize = readLE(2)
  ...
  return safeResult({
    value: {
      ...
      heapZeroPaddingSize: heapZeroPaddingSize,
      stackSize: stackSize,
      roData,
      rwData,
      codeSize: codeLength,
...
> {
  ...
  return safeResult({
    value: {
      ...
      heapZeroPaddingSize: programResult.value.heapZeroPaddingSize,
