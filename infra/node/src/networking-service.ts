...
interface ChainSpec {
  id: string
  bootnodes?: string[]
  genesis_validators?: ChainValidator[]
}
...
export class NetworkingService extends BaseService {
  ...
  constructor(
    ...
    this.config = config
    this.networkingStore = networkingStore
    ...
    this.workPackageSubmissionProtocol = new WorkPackageSubmissionProtocol()
  ...
  private parseBootnode(bootnode: string): Safe<BootnodeInfo> {
    ...
    return safeResult({ peerId, host, port })
  }
        ...
        if (configData.genesis_validators) {
          return configData.genesis_validators.map(
            (validator: ChainValidator, index: number) => ({
              ...validator,
              validator_index: validator.validator_index ?? index,
            }),
          )
        }
  ...
  async init(): Promise<void> {
    try {
      ...
      try {
        logger.info('Starting QUIC transport initialization')
  ...
  async start(): Promise<boolean> {
    try {
      ...
      logger.info(
        `Loaded ${chainValidators.length} validators from chain config`,
      )
      ...
      for (const chainValidator of chainValidators) {
        ...
        const port = Number.parseInt(portStr || '30333')
        ...
        const validator: ValidatorMetadata = {
          index: chainValidator.validator_index || 0,
      ...
      logger.info(
        'Starting block announcement protocol from networking service',
      )
  ...
  async stop(): Promise<void> {
    try {
      logger.info('Stopping JIP-5 compliant networking service')
  ...
  private extractStreamInfo(streamId: string): Safe<{
    ...
    const validatorIndex = Number.parseInt(validatorIndexStr, 10)
