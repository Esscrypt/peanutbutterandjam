...
import { parseGenesisJson } from '@pbnj/core'
...
describe('StateService - convertToMapping', () => {
  ...
  beforeEach(() => {
    ...
    const genesisManagerService = new NodeGenesisManager(configService, undefined, {
      genesisJsonPath: genesisPath,
    })
    const entropyService = new EntropyService(configService)
    const ticketHolderService = new TicketService(configService)
    const validatorSetManager = new ValidatorSetManager(configService)
    stateService = new StateService({
      genesisManagerService: new NodeGenesisManager(configService),
      entropyService: new EntropyService(configService),
      ticketHolderService: new TicketService(configService),
      configService: configService,
      validatorSetManager: new ValidatorSetManager(configService),
    })
  })
