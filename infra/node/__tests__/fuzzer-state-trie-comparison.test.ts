...
describe('Fuzzer State Trie Comparison', async () => {
  it('should compare generateStateTrie with fuzzer keyvals and identify differences', async () => {
    ...
    console.log(`✅ State set from ${init.keyvals.length} keyvals (some may have been skipped due to decoding errors)`)
    ...
    console.log(`\n✅ Comparison complete`)
    console.log(`  Keyvals root matches expected: ${keyvalsRoot === expectedRoot ? '✅' : '❌'}`)
    console.log(`  Generated trie root matches expected: ${generatedRoot === expectedRoot ? '✅' : '❌'}`)
    console.log(`  StateService root matches expected: ${stateServiceRoot === expectedRoot ? '✅' : '❌'}`)
