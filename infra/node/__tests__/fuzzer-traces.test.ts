...
describe('Fuzzer Traces Test', () => {
  ...
  it('should import trace file 1766241814 from fuzz-reports', async () => {
    ...
    // Extract validators from first block's epoch mark if present
    if (firstTraceData.block?.header?.epoch_mark?.validators) {
      const epochMarkValidators = firstTraceData.block.header.epoch_mark.validators.map((validator: any) => ({
        bandersnatch: validator.bandersnatch,
        ed25519: validator.ed25519,
        bls: validator.bls || '0x' + '00'.repeat(144),
        metadata: validator.metadata || '0x' + '00'.repeat(128),
      }))
      
      // Get ValidatorSetManager from stateService and set validators
      // This ensures validators are available for epoch root verification
      const validatorSetManager = (stateService as any).validatorSetManager
      if (validatorSetManager) {
        // Set active set from epoch mark validators
        // The pre-state will override these when setState is called, but this ensures
        // they're available for validation of the first block
        validatorSetManager.setActiveSet(epochMarkValidators)
        console.log(`📋 Set ${epochMarkValidators.length} validators from first block's epoch mark on ValidatorSetManager`)
      }
    } else {
      console.log(`⚠️  First block does not have epoch mark with validators`)
    }
    ...
    for (const traceFile of filteredTraceFiles) {
      ...
      if (blockNum === startBlock) {
        // Set pre-state from starting block's trace
