...
describe('Genesis Parse Tests', () => {
  ...
  describe('Safrole Genesis', () => {
    it('should parse genesis.json from traces/storage_light', async () => {
      ...
      const services = await initializeServices({ spec: 'tiny', traceSubfolder: 'storage', genesisManager, initialValidators, useWasm: true, useIetfVrfWasm: true })
      ...
      const parseStateKeyForDebug = (keyHex: Hex) => {
        ...
        if (error) {
          // If parsing fails, try to manually decode the key structure
          const keyBytes = hexToBytes(keyHex)
          if (keyBytes.length !== 31) {
            return { error: `Invalid key length: ${keyBytes.length} bytes (expected 31)` }
          }
          
          const firstByte = keyBytes[0]
          
          // Check if it might be a C(s, h) key (service storage/preimage/request)
          // C(s, h) = ⟨n₀, a₀, n₁, a₁, n₂, a₂, n₃, a₃, a₄, a₅, ..., a₂₆⟩
          // where n = encode[4](s), a = blake(h)
          if (firstByte !== 0 && firstByte !== 0xff && (firstByte < 1 || firstByte > 16)) {
            ...
            return {
              ...
              hash: blakeHash,
              note: 'Service storage/preimage/request key (manually parsed)'
            }
          }
          
          return { error: error.message, firstByte: firstByte.toString(16) }
        }
        ...
        for (const keyval of blockJsonData.post_state.keyvals) {
          ...
          if (keyval.value !== expectedValue) {
            ...
            if ('error' in keyInfo) {
              ...
              const serviceKeyInfo = keyInfo as { serviceId: bigint | string; type?: string; hash?: Hex | string }
