 ...
 *
 * Tests processing of individual trace files from w3f-jam-conformance fuzz-reports
...
const TRACES_DIR = path.join(WORKSPACE_ROOT, 'submodules/w3f-jam-conformance/fuzz-reports', JAM_CONFORMANCE_VERSION, 'traces')

// Mismatch logs directory
const MISMATCH_LOGS_DIR = path.join(WORKSPACE_ROOT, 'mismatch-logs/w3f-jam-conformance')
...
describe('W3F JAM Conformance Traces', () => {
  const configService = new ConfigService('tiny')

  // Log the version being used
  console.log(`\n📦 W3F JAM Conformance Version: ${JAM_CONFORMANCE_VERSION}`)
  ...
  afterAll(() => {
    ...
    console.log('\n' + '='.repeat(80))
    console.log('📊 W3F JAM CONFORMANCE TRACES - FINAL SUMMARY')
  ...
  for (const [traceDir, traceFiles] of traceFilesByDir.entries()) {
    ...
    for (const traceFilePath of traceFiles) {
      ...
      it(`should process trace ${relativePathWithoutExt}`, async () => {
        ...
        // Include version in the path: pvm-traces/w3f-jam-conformance/{version}/{relative_path}
        ...
        // Include version in the path: w3f-jam-conformance/{version}/{relative_path}
        ...
        services = await initializeServices({ spec: 'tiny', traceSubfolder, genesisManager, initialValidators, useRust: true, useWorkerPool: false, useRingVrfWasm: true })
