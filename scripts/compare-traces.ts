 ...
 * from the jamduna submodule and analyzes differences.
 *
...
function analyzeRootCause(
  ...
  if (!firstDiff) {
    analyses.push('✅ No differences found - traces match perfectly!')
    return analyses
  }

  // Analyze the first difference
  analyses.push(`${colors.bold}🔍 Root Cause Analysis${colors.reset}`)
  analyses.push('')
  ...
  if (firstDiff.stepNumber === 1) {
    ...
    if (ourRegs && refRegs) {
      ...
      if (regDiffs.length > 0) {
        analyses.push(
          `${colors.yellow}⚠️  Initial register state mismatch in: ${regDiffs.join(', ')}${colors.reset}`,
        )
        analyses.push(
    ...
    if (firstDiff.ourLine?.gas !== firstDiff.refLine?.gas) {
      analyses.push(`${colors.yellow}⚠️  Initial gas mismatch${colors.reset}`)
  ...
  if (pcDiffs.length > 0) {
    analyses.push(
      `${colors.cyan}📍 PC Divergence detected at step ${pcDiffs[0].step}${colors.reset}`,
    )
  ...
  if (instrDiffs.length > 0) {
    analyses.push(
      `${colors.magenta}🔀 Instruction mismatch at step ${instrDiffs[0].step}${colors.reset}`,
    )
  ...
  if (gasDiffs.length > 0 && pcDiffs.length === 0 && instrDiffs.length === 0) {
    analyses.push(
      `${colors.yellow}⛽ Gas consumption difference at step ${gasDiffs[0].step}${colors.reset}`,
    )
  ...
  if (regDiffs.length > 0 && pcDiffs.length === 0) {
    analyses.push(
      `${colors.blue}📊 Register value difference at step ${regDiffs[0].step}${colors.reset}`,
    )
  ...
  if (
    ...
    if (missingOurs.length > 0) {
      analyses.push(
        `${colors.red}❌ Our trace is shorter by ${missingOurs.length} steps${colors.reset}`,
      )
      analyses.push('   Possible causes:')
    ...
    if (missingRefs.length > 0) {
      analyses.push(
        `${colors.green}➕ Our trace is longer by ${missingRefs.length} steps${colors.reset}`,
      )
      analyses.push('   Possible causes:')
...
function printComparison(
  ...
  console.log(
    `${colors.bold}📊 Trace Comparison for Block ${blockNumber}${colors.reset}`,
  )
  console.log(
    `${colors.bold}═══════════════════════════════════════════════════════════════${colors.reset}`,
  )
  ...
  if (result.firstDiff) {
    console.log(
      `${colors.bold}🔴 First Difference at Step ${result.firstDiff.stepNumber}${colors.reset}`,
    )
    console.log()
...
function findTraceFiles(
  ...
  // Reference path (jamduna)
  const refPath = join(
    workspaceRoot,
    'submodules/jamduna/jam-test-vectors/0.7.1/preimages_light',
  ...
  if (executorType) {
    ...
    // Auto-detect: prefer typescript, then wasm, then jamduna format
    const patterns = [
...
function main() {
  ...
  if (!refPath) {
    ...
    console.error(
      'Expected at: submodules/jamduna/jam-test-vectors/0.7.1/preimages_light/',
    )
    process.exit(1)
  }
